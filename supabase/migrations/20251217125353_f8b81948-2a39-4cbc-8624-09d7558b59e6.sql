-- Mettre à jour la fonction approve_rental_request pour calculer la caution comme 5x loyer
...
BEGIN
  ...
  -- Calculer la caution comme 5x le loyer mensuel
  -- (2 mois avance + 2 mois garantie + 1 mois démarcheur)
  v_calculated_caution := p_monthly_rent * 5;
  ...
  INSERT INTO public.leases (
    ...
    p_start_date,
    p_monthly_rent,
    v_calculated_caution,
    false,
    'actif'
  )
  RETURNING id INTO v_new_lease_id;

  -- 5. Créer la facture de caution (premier paiement obligatoire)
  ...
  INSERT INTO public.notifications (
    ...
    'Félicitations ! Votre demande pour "' || v_property.titre || '" a été approuvée. Vous devez payer la caution de ' || v_calculated_caution || ' FCFA (5x loyer: 2 mois avance + 2 mois garantie + 1 mois démarcheur). Après paiement, votre premier loyer sera dû dans 2 mois.'
  );
  ...
  v_result := json_build_object(
    ...
    'caution_amount', v_calculated_caution,
    'message', 'Attribution réussie, bail créé et caution facturée (5x loyer mensuel)'
  );
